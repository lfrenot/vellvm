...
Definition denote_ocfg_equiv_cond (g1 g2 g2': ocfg) (TO :gset bid) (σ: bid -> bid) :=
...
Proof.
  ...
  intros * NIN IH * IN. cbn in *.
  (* rewrite map_fold_insert; auto. *)
  rewrite elem_of_dom in IN |- *. destruct IN as [b' IN].
  case (decide (id=b)).
  - intros <-. 
  phi_rename_correct
Admitted. *)
  ...
  - intros IN.
    ...
    + unfold dom_phi, phi_rename in IN |- *. rewrite map_fold_insert.
      ...
      -- unfold exps_rename. intros j1 j2 * NEQj M1 M2.
         case (decide (σ j1 = σ j2)) as [EQ|nEQ].
...
Proof.
  ...
  - intros * NIN REC * [SAFE1 SAFE2]. cbn in *. unfold exps_rename in *.
    rewrite map_fold_insert.
    * case (decide (b = i)) as [EQ|NEQ]. 2: case (decide (σ b = σ i)) as [EQσ|NEQσ].
      + subst. rewrite lookup_union. rewrite lookup_singleton. rewrite union_Some_l. apply lookup_insert.
      + forward SAFE2. {
          rewrite EQσ. rewrite map_fold_insert. set_solver.
          - constructor; auto. intros ? * ? ?. now subst.
          - intros * ? * ?. now subst.
          - intros * NEQ' M1 M2. case (decide (σ j1 = σ j2)) as [EQj|NEQj].
            * pose proof SAFE1 _ _ _ _ M1 M2 EQj as EQz. now rewrite EQj, EQz.
            * rewrite <- !insert_union_singleton_l. now apply insert_commute.
          - trivial.
        } 
...
Proof.
  ...
  - reflexivity.
  - destruct φ as [x φ]. cbn.
    match goal with
      |- context[raise ?s] => generalize s
    end. intros. case_match.
    pose proof phi_rename_correct args t σ from as EQ.
    forward EQ. now inversion HSafe.
    forward IH. now inversion HSafe.
    apply eutt_clo_bind with (UU := Logic.eq); cycle 1.
    now intros ? ? <-.
    apply eutt_clo_bind with (UU := Logic.eq).
    unfold phi_rename. now rewrite EQ.
    intros [] ? <-.
    apply eutt_clo_bind with (UU := Logic.eq); cycle 1.
    now intros ? ? <-.
    cbn. rewrite MapMonadExtra.map_monad_map.
    unfold denote_phi.
    match goal with
      |- context[raise ?s] => generalize s
    end. intros.
       (* case_match.
    * case_match. inversion H1. rewrite <- EQ. cbn in IH. apply eutt_clo_bind with (UU := Logic.eq). *)
...
Definition find_block_some_ocfg_rename_P g := forall id b σ, g !! id = Some b ->
  (ocfg_rename σ g) !! id = Some (bk_phi_rename σ b).

Lemma find_block_some_ocfg_rename: forall g, find_block_some_ocfg_rename_P g.
Proof.
  intros. apply map_ind; unfold find_block_some_ocfg_rename_P. now cbn.
  intros * NIN REC * IN.
  apply lookup_insert_Some in IN as [[<- <-] | [NEQ IN]]; unfold ocfg_rename; now simplify_map_eq.
Qed.

Definition σbksSafe σ (g: ocfg) from : Prop :=
  map_Forall (fun (id:bid) => fun x => σφsSafe σ (x.(blk_phis)) from) g.

(* Print emptyunion. *)

Lemma in_outgoing_bks (g: ocfg) (b: bid) (l:gset bid): b ∈ set_fold (fun id acc => predecessors id g ∪ acc) ∅ l -> exists id, id ∈ l /\ b ∈ predecessors id g.
Proof.
  apply set_fold_ind with (X:=l).
  intros ? ? * EQ ?.
Admitted.
...
Proof.
  ...
  ecofix cih.
  clear cihH.
  intros EQ DIS DISσ DOMσ CND * EQσ NINt NINf SAFE.
  remember ((outputs g1) ∩ (inputs g2)) as TO.
  (* pose proof cihL EQ CND. *)
  (* Either we are in the 'visible' graph or not. *)
  case (decide (to ∈ (TO ∪ inputs g1))) as [tIN'|tNIN'].
  (* Either we enter g1 or not *)
  - apply elem_of_union in tIN' as [tIN|tIN]; cycle 1.
    * (* if we enter g1: then process [g1], and get back to the whole thing *)
      ...
      assert (FIND': (ocfg_rename σ g1 ∪ g2') !! to = Some (bk_phi_rename σ bk)) by now simplify_map_eq.
      rewrite denote_ocfg_in_eq_itree; [| exact FIND'].
